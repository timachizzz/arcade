...
class Deadline(arcade.View):
    ...
    def on_draw(self):
        ...
        for i in range(self.bombs):
            arcade.draw_texture_rect(
                self.bomb_texture,
                arcade.rect.XYWH(self.width // 2 + (i + 6) * self.bomb_texture.width, self.height * 0.945,
                                 self.bomb_texture.width, self.bomb_texture.height)
            )
        self.batch.draw()
    ...
    def on_update(self, delta_time):
        ...
        text = f'{int(self.timer // 60)}:{int(self.timer % 60):02d}'
        if self.timer <= 15:
            text = f'{int(self.timer)}:{int(self.timer % 1 * 100):02d}'
        self.timer_text = arcade.Text(
            text,
            self.width // 2, self.height * 0.92,
            font_size=40, align='center', anchor_x='center',
            batch=self.batch
        )
        ...
        if not self.alive:
            ...
            if self.enemies_list:
                ...
                if self.timer == 0:
                    self.enemies_list.clear()
                elif self.enemies_list[0].blinking_times == 0:
                    if self.timer > 0:
                        self.reset()
                    elif not self.emitters:
                        self.enemies_list.clear()
            elif self.timer == 0 and not self.emitters:
                self.gui_camera.use()
                game_over_view = GameOverView(self.score, self.highscore, self.__class__.__name__)
                self.window.show_view(game_over_view)
            return
        ...
        self.doubler_text = arcade.Text(
            f"x{self.score_multiplier}",
            self.width // 2 * 0.8, self.height * 0.95,
        ...
        if enemy := check_for_collision_with_list(self.player, self.enemies_list):  # Death
            self.enemies_list.clear()
            self.non_touchable_enemies_list.clear()
            self.doublers_list.clear()
            self.bullet_list.clear()
